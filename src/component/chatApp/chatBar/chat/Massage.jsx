...
<>
{
  chat.map(
    massage => (
  <MassageContainer key={massage.id} isUser= {massage.isSeen}>
    <Image src={ massage.isSeen ? "/chat.png" : user.picture.thumbnail} alt={user.name.last} width={22} height={22} style={{borderRadius: '50%'}}/>
    <ItemContainer>
        {
          massage.text.map( text => (
            <MassageItem key={`${massage.id} ${text}`} isUser= {massage.isSeen}>
               <TextSecondry>
                {text}
               </TextSecondry>
           </MassageItem>
